...
\Comment{Edges format consists of pointer to other Node it is connected to and a pointer to its counterpart in the other Node.}
\For{$x \in X$}
...
\While{X has edges}
\If {numOfEdges(peek($X$)) $>$ numOfEdges(peek($Y$))}
\State $deleteEdges(pop(X))$
\Comment{Sort included in delete edges. Swap nodes upon removal of edge if next node has more edges.}
\Else
...
\begin{algorithm}[H]
\caption{Minimum Vertex Coverage (Not-Greedy)}
\begin{algorithmic}
\State Let $X$ = set of all nodes
\While{X has edges}
\State $x$ = random node from $X$
\State $y$ = random connected node to $x$
\State $delete$ $x$,$y$ from $X$
\EndWhile
\end{algorithmic}
\end{algorithm}
